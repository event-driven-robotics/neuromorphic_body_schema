...
class Ik_solver:
    def __init__(self,model,data,joint_name: List[str],end_name:str,option:str,damp=0.15,alpha=0.5):
        """
        ...
        assert option in ["euler","quat"],print("should chooese between euler or quat")
        self.option=option
    ...
    def check_limites(self,joint):
        ...
        for j,index in zip(joint,self.joint_id):
                j=max(self.model.jnt_range[index][0],min(j,self.model.jnt_range[index][1]))
                joints.append(j)
    ...
    def ik_step(self, target_pos, target_ori, max_iter=10000, pos_thres=0.001, ori_thres=0.001):
        """
        ...
        for i in range(max_iter):
            ...
            mujoco.mj_jac(self.model,self.data,self.jacp,self.jacr,target_pos,self.ee_id)
...
#     joint_pose={joint_name:pose for joint_name,pose in zip(joint_name,q_arm)}
