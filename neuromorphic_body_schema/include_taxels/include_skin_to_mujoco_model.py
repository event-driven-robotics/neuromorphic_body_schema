...
def include_skin_to_mujoco_model(mujoco_model, path_to_skin, skin_parts):
    ...
    ini_files_taxels = [f for f in ini_files_taxels if f.split(".")[0] in skin_parts]
    all_taxels = []
        ...
        if "<body name=" in lines[line_counter]:
            if any(part in lines[line_counter] for part in MOJOCO_SKIN_PARTS):
                ...
                if part == "r_upper_leg":
                    ...
                    # find the corresponding taxels
                    pos_of_taxels = ini_files_taxels.index("right_forearm_V2.txt")
                    taxels_to_add = all_taxels[pos_of_taxels]
                    found_spot_to_add = False
                    add_taxels = True
                    group_counter = 2
                    ...
                    # find the corresponding taxels
                    part = "r_palm"
                    pos_of_taxels = ini_files_taxels.index("right_hand_V2_1.txt")
                    taxels_to_add = all_taxels[pos_of_taxels]
                    found_spot_to_add = False
                    add_taxels = True
                    group_counter = 3
                    ...
                    # find the corresponding taxels
                    pos_of_taxels = ini_files_taxels.index("left_forearm_V2.txt")
                    taxels_to_add = all_taxels[pos_of_taxels]
                    found_spot_to_add = False
                    add_taxels = True
                    group_counter = 5
                    ...
                    # find the corresponding taxels
                    part = "l_palm"
                    pos_of_taxels = ini_files_taxels.index("left_hand_V2_1.txt")
                    taxels_to_add = all_taxels[pos_of_taxels]
                    found_spot_to_add = False
                    add_taxels = True
                    group_counter = 6
                    ...
                    if add_finger_taxels:
                        while not found_spot_to_add:
                            ...
                            if "</body" in lines[line_counter]:
                                ...
                                identation = len(lines[line_counter].split("<")[0]) + 8
    ...
    while keep_processing:
        ...
        if "<contact>" in lines[line_counter]:
            for part_to_add, taxels_to_add in zip(parts_to_add, taxel_ids_to_add):
                ...
                line_counter += 1
                lines.insert(line_counter, f'{" "*identation}<!--{part_to_add}-->\n')
                line_counter += 1
