...
def get_joint_dof_num(model, joint_id):
...
def get_dof_indices(model, joint_names):
    """
    Returns the indices of the DOFs corresponding to the given joint names.
    ...
    Returns:
        List of DOF indices for each joint name.
    """
    dof_indices = []
    for name in joint_names:
        ...
        # Get the DOF address and number of DOFs for this joint
        dof_adr = model.jnt_dofadr[joint_id]
        dof_num = get_joint_dof_num(model, joint_id)
        # Append all DOF indices for this joint
        dof_indices.extend(range(dof_adr, dof_adr + dof_num))
    return dof_indices
...
def qpos_from_site_pose(model,
                        ...
                        max_update_norm=2.0,
                        progress_thresh=20.0,  # 20.0
                        max_steps=500,
                        inplace=False) -> IKResult:
    ...
    if not inplace:
      spec = mujoco.mjtState.mjSTATE_FULLPHYSICS  # or mjSTATE_PHYSICS, etc.
    ...
    for steps in range(max_steps):
        ...
        if err_norm < tol:
            ...
            update_joints = _nullspace_method(
            ...
            if progress_criterion > progress_thresh:
                logging.info('Step %2i: err_norm / update_norm (%3g) > '
            ...
            logging.info('Step %2i: err_norm=%-10.3g update_norm=%-10.3g',
...
def _nullspace_method(jac_joints, delta, regularization_strength=0.0):
